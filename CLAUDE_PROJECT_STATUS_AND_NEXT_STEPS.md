...
**Not verified (needs credentials):** real Live Score API and GameForecastAPI responses. Both integrations follow the published documentation/spec; the first live run must be watched (`/api/v1/data-providers/status` shows budgets and last errors). The data-source evaluation in `docs/research/` was corrected: football-data.co.uk prohibits commercial and AI-training use, so it is not a training source; no in-house model is built in this phase.
